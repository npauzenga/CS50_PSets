...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // if request-target (after method) does not start with "/", 501
            if(line[4] != "/")
            {
                error(501);
            }
            ...
            char httpVersion[9];
            char requestTarget[50];
            int spaces = 0;
            
            for(int i = 0, j = strlen(line); i < j; i++)
            {
                // if request-target contains ", 400
                if(line[i] == "\"")
                {
                    error(400);
                }
                
                // look for first space, after will be request-target
                if(line[i] == " ")
                {
                    // count spaces to find the next one
                    spaces++;
                    
                    int k = i + 1;
                    while(line[k] != " " && requestTarget[0] != "/")
                    {
                        strcpy(line[k], requestTarget);
                        k++;
                    }
            ...
            // if absolute-path does not contain ".", 501
...
const char* lookup(const char* extension)
{
    ...
    for(int i = 0, j = strlen(extension); i <= j; i ++)
    {
        lowercaseExtension[i] = tolower(extension[i]);
    }
    
    int length = strlen(lowercaseExtension);
    
    
    if (strncmp(lowercaseExtension, ".css", length) == 0)
        return "text/css";
    else if (strncmp(lowercaseExtension, ".html", length) == 0)
        return "text/html";
    else if (strncmp(lowercaseExtension, ".gif", length) == 0)
        return "image/gif";
    else if (strncmp(lowercaseExtension, ".ico", length) == 0)
        return "image/x-icon";
    else if (strncmp(lowercaseExtension, ".jpg", length) == 0)
        return "image/jpeg";
    else if (strncmp(lowercaseExtension, ".js", length) == 0)
        return "text/javascript";
    else if (strncmp(lowercaseExtension, ".png", length) == 0)
        return "image/png";
    else    
        return NULL;
}
